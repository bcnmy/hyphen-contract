    ...
    ERC2771ContextUpgradeable
{
    ...
    // Free transactions could be given reaching upon particular milestone code
    //mapping(address => uint256) public feeRebates;

    // Milestones availed by user on this pool
    //mapping(uint256 => mapping(address => bool)) public availedMilestones;

    // Mmoved off chain as needed cumulative from all pools
    ...
    function setTxnCount(address _user, uint256 _count) external onlyOwner {
        txnCount[_user] = _count;
    }

    function setDepositVolume(address _user, address _token, uint256 _amount) external onlyOwner {
     ...
     */
    //@notice Users might provide intents in next versions
    ...
    function depositNative(
        ...
        emit Deposit(sender, NATIVE, receiver, toChainId, msg.value + rewardAmount, rewardAmount, tag, mileStoneCode);
    }

    //@notice Might later pass more information about deposit hash receipt
    function sendFundsToUser(
        ...
        //@review if we pass mileStoneCode this can be part of checkHashStatus -> hashSendTransaction

        require(!status, "Already Processed");
        processedHash[hashSendTransaction] = true;

        // Update milestone availed by user on this pool
        /*if (mileStoneCode != 0) {
        ...
        /*if(tokenGasPrice == 0) {
            // Mark number of fee rebates for sender/receiver , processed hash or token addresss
        }*/
