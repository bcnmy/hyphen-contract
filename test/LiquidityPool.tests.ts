...
describe("LiquidityPoolTests", function () {
    ...
    //         receiver, toChainId, tokenValue, 0, tag);
    ...
    //         receiver, toChainId, calculatedRewardAmount.add(amountToDeposit).toString(), calculatedRewardAmount, tag);
    ...
    it("Real world use case for Native currency, when deposit gets all incentive pool", async () => {
        ...
        const amountToDeposit = BigNumber.from(minTokenCap).add(BigNumber.from(getLocaleString(5 * 1e18))).toString();
        ...
        expect(depositTx).to.emit(liquidityPool, DEPOSIT_EVENT).withArgs(await getOwnerAddress(), NATIVE,
            receiver, toChainId, calculatedRewardAmount.add(amountToDeposit).toString(), calculatedRewardAmount, tag);
    })
    ...
    it("Real world use case for Native currency, when deposit gets some part of incentive pool", async () => {
        ...
        const amountToWithdraw = BigNumber.from(minTokenCap).add(BigNumber.from(getLocaleString(5 * 1e18))).toString();
        let receiver = await getReceiverAddress();
        let toChainId = 1;
        await executorManager.addExecutor(executor.address);
        // Send funds to put pool into deficit state so current liquidity < provided liquidity
